...
export async function stageChronicle(
  target = resolve(import.meta.dirname, "../.generated/chronicle"),
) {
  // A removed dependency must not survive locally and hide a clean-deploy failure.
  ...
  const entries = [
    ...
    "generate.mjs",
    "replay.mjs",
  ].map((name) => resolve(repo, "examples/chronicle", name));
  ...
  while (pending.length) {
    ...
    for (const match of source.matchAll(
      /new URL\(\s*["'](\.{1,2}\/[^"']+\.mjs)["']\s*,\s*import\.meta\.url\s*,?\s*\)/g,
    ))
...
export function collectReplay(url: string, collector: ChronicleDigest, options?: { runtime?: string | ((buildId: string) => Promise<string | undefined> | string | undefined); runtimeOrigin?: string; signal?: AbortSignal; lookup?: (lookup: (name: string) => Promise<any>) => Promise<void>; maxInputs?: number; onProgress?: (done: number, total: number) => void; onTiming?: (timing: ReplayTiming) => void }): Promise<Digest>;`,
  ...
  return [...copied].map((f) => relative(repo, f)).sort();
}
if (
  process.argv[1] &&
  import.meta.url === new URL("file://" + process.argv[1]).href
) {
  const staged = await stageChronicle();
  console.log(
    `Staged ${staged.length} chronicle modules into .generated/chronicle`,
  );
}
