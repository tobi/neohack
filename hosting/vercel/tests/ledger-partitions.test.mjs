...
test('partitioned ledger preserves published records and rebuilds bounded summaries',async()=>{
 ...
 await storageContext.run(store,async()=>{
  ...
  const stats=await ledgerStats();assert.equal(stats.totals.runs,25);assert.equal(stats.totals.longest,0,'unrecorded progress is listed, not ranked');assert.equal(stats.recent.length,25);
