...
- Latest green checks: `bun test mcp/tests` = **53 tests / 670 assertions**; native `test_explorer` green; MCP smoke PASS; broader MCP acceptance **15 pass / 0 skip**.
...
- Main server restarted under our ownership in Herdr pane **w2G:p8**, port **3000**, normal repo session root. At last check PID **3229558** (verify identity before stopping). Log `/tmp/ascent/takeover/front.log`.
- Older candidate server in pane **w2G:p7**, port **3311**, sessions `/tmp/ascent/takeover/dev-sessions`; it is stale. Current reconstruction candidate is pane **w2G:pC**, port **3312**, sessions `/tmp/ascent/reconstruction/browser-sessions`, last PID **3215524** (verify before stopping).
...
2. Prioritize recording/index crash recovery, keeping durable request reservations fail-closed. Add raw native UTF-8/parser fuzzing and broader action-context checks; explicit life-saving and other occupation fixtures remain unstaged. Audit perception freshness for older engine pins rather than pretending they emit the new facts.
