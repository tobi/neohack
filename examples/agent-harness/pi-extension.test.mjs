...
test('Pi binding registers only discovered tools, passes exact arguments, reports active tools and model limit', async t=>{
  ...
  t.mock.method(globalThis,'fetch',async(url,options)=>{
    ...
    return {ok:true,json:async()=>url.endsWith('/tools')?{tools:[{name:'session_observe',description:'Observe',inputSchema:schema}]}:{result:'fixture'}};
  });
  await extension({registerTool:tool=>registered.push(tool),on:(event,fn)=>handlers.set(event,fn),getActiveTools:()=>['session_observe']});
  assert.deepEqual(registered.map(t=>t.name),['session_observe']);
  ...
  assert.deepEqual(JSON.parse(calls[2].options.body),{name:'session_observe',arguments:{}});
