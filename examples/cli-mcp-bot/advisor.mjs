...
export function recommend(frame) {
  ...
  if(!observation)return suggest(2,"observe",{},'Read the current perceived scene.');
  const vitals=observation.vitals;
  if(['hungry','weak','fainting','fainted','starved'].includes(vitals?.hunger))
    return suggest(3,"eat",{},'Ask the engine for edible candidates; choose deliberately. No corpse is classified as safe by this advisor.');
  ...
  if(creature)return suggest(4,"inspect",{target:{x:creature.x,y:creature.y}},'Inspect the perceived creature and available actions. Use lookup for game lore; attack remains an explicit choice.');
