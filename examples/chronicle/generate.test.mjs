...
import { MODEL, SYSTEM } from "./prompt.mjs";
...
test("gatewayStory sends one bounded request with the caller's token and never retries a failure", async () => {
  ...
  const fetchDouble = async (url, init) => {
    ...
    return okResponse({ choices: [{ message: { content: "```json\n" + JSON.stringify(story(d)) + "\n```" }, finish_reason: "stop" }], usage: { total_tokens: 10 } });
  };
  ...
  assert.deepEqual(JSON.parse(body.messages[1].content), JSON.parse(JSON.stringify(d)), "the user message is exactly the evidence packet");
...
test("parseStory accepts fenced or bare JSON and rejects invalid or uncited stories", () => {
  const d = digest();
  ...
  const uncited = story(d);
  uncited.paragraphs[1].sources = ["e999"];
  assert.throws(() => parseStory(JSON.stringify(uncited), d), /source/);
});
