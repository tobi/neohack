...
import { parseWireJson, MAX_WIRE_BYTES } from "./wire-json";

// Do not strip unknown fields: that can silently turn a malformed intent into
// a valid but different action. Shapes and game semantics belong to the C core.
export function forwardToolRequest(
  tool: string,
  args: unknown,
): Record<string, unknown> {
  if (!args || typeof args !== "object" || Array.isArray(args))
    throw new Error("tool arguments must be an object");
  if (Object.hasOwn(args, "tool"))
    throw new Error("tool is a reserved transport field");
  return { ...args, tool };
}
...
export const TOOLS = [
    ...
    inputSchema: {
      ...
      properties: {
        seed: {
          type: "integer",
          description:
            "Fixed fate for a replayable world; omit for a fresh one.",
        },
        name: { type: "string", description: "Your name." },
        role: {
          type: "string",
          description:
            "Your calling: archeologist, barbarian, caveman, healer, knight, monk, priest, rogue, ranger, samurai, tourist, valkyrie, wizard.",
        },
        race: {
          type: "string",
          description: "Your folk: human, elf, dwarf, gnome, orc.",
        },
        gender: { type: "string", description: "male or female." },
        align: {
          type: "string",
          description: "Your heart: lawful, neutral, chaotic.",
        },
      },
    },
  },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        action: {
          type: "string",
          description:
            "What to do: move, wait, climb, inspect, inventory, search, kick, open, close, pickup, eat, drink, wield, equip, wear, remove, read, apply, drop, zap, pray.",
        },
        direction: {
          type: "string",
          description:
            "Compass direction (or up/down): north, south, east, west, northeast, northwest, southeast, southwest, up, down.",
        },
        target: {
          description: 'Where to aim: "self", "here", or {direction: ...}.',
        },
        item: {
          description: "What to use: a perceived name, or {id} from a listing.",
        },
        replyTo: {
          type: "string",
          description: "The standing decision id being answered.",
        },
        confirm: {
          type: "boolean",
          description: "Answer a confirmation decision.",
        },
        choose: {
          description: "Answer a choice decision: option id, or array of ids.",
        },
        text: {
          type: "string",
          description: "Answer a text decision with words.",
        },
        cancel: {
          type: "boolean",
          description: "Turn away from the standing decision.",
        },
        expectedRevision: {
          type: "integer",
          description: "Fail with staleRevision when the world moved.",
        },
        requestId: {
          type: "string",
          description:
            "Client-generated id: same id+payload replays the original result instead of acting twice.",
        },
      },
      required: ["sessionId"],
    },
  },
  {
    name: "resume",
    description:
      "Replay captured inputs with the world's pinned engine and return the resulting observation and standing decision. Resume is interactive; use the archive viewer for engine-free review.",
    inputSchema: {
      type: "object",
      properties: { sessionId: { type: "string" } },
      required: ["sessionId"],
...
export async function callTool(
  ...
  switch (name) {
    ...
    case "resume":
    case "end_session":
      return callCore(forwardToolRequest(name, args));
    default:
  ...
  try {
    if (msg.method === "initialize") {
      return ok({
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "neonethack-mcp", version: "0.6.0" },
      });
      ...
      if (typeof name !== "string")
        return fail(-32602, "tools/call needs a string name");
      const args = (
...
async function main(): Promise<void> {
  ...
  for await (const chunk of Bun.stdin.stream()) {
    ...
    while ((idx = buf.indexOf("\n")) >= 0) {
      ...
      try {
        await handleMessage(parseWireJson(line));
      } catch {
        process.stdout.write(
          JSON.stringify({
            jsonrpc: "2.0",
            id: null,
            error: { code: -32700, message: "invalid or ambiguous JSON frame" },
          }) + "\n",
        );
      }
    }
    if (Buffer.byteLength(buf, "utf8") > MAX_WIRE_BYTES)
      throw Error("unterminated JSON frame exceeds the transport limit");
  }
}
