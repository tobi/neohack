...
<span class="code-keyword">try</span> {
  ...
  private async inspectTile(x: number, y: number, obstruction = false, walkImmediately = false) {
    ...
    const loreName = occupant ? occupant.appearance ?? "" : (cell.terrain?.type.replace(/([A-Z])/g, " $1").toLowerCase() ?? "");
    ...
    panel.querySelector("p")!.textContent =
      occupant && occupant.kind !== "self"
        ? (occupant.kind === "ally"
          ...
          (occupant.appearance
            ? ""
            : " Only a creature category is perceived; the illustration does not identify it.")
