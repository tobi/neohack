...
search/rest, up to 1,000 turns. An editable box beneath the hero shows clickable completions. Backspace edits; Escape cancels. More actions (`Ctrl+K`, `Cmd+K` on Mac, or `#`) opens focused fuzzy search on keyboard or touch: match action names and commands together, then press Enter or tap a result to try it. Search `auto:explore` or `auto:descend` for bounded perceived-map walking (not vanilla NetHack commands); Escape stops the leg. Item and spell choices appear in the same dialog, with Escape returning to the parent search. Typing never executes by itself; targets and warnings remain explicit.
