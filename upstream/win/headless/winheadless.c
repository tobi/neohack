/* neonethack headless port: window_procs -> NDJSON JSON-RPC (see PROTOCOL.md).
 *
...
static void
hl_emit_glyph_obj(JBuf *jb, const glyph_info *gi, const glyph_info *bg)
{
    jb_begin_obj(jb);
    jb_key(jb, "glyph");
    jb_int(jb, gi ? gi->glyph : NO_GLYPH);
    jb_key(jb, "ttychar");
    jb_int(jb, gi ? gi->ttychar : 0);
    jb_key(jb, "framecolor");
    jb_int(jb, gi ? (long long) gi->framecolor : 0);
    jb_key(jb, "tileidx");
    jb_int(jb, gi ? gi->gm.tileidx : 0);
    jb_key(jb, "color256idx");
    jb_int(jb, gi ? gi->gm.color256idx : 0);
    if (bg) {
        jb_key(jb, "bkglyph");
        jb_int(jb, bg->glyph);
    }
...
hl_answer_new_game(long long id, const char *params)
{
    ...
    if (hl_have_seed) {
         ...
         * logged seed. This is what makes resume-by-replay bit-exact. */
        hl_seed_calls = 0;
        init_random(rn2);
...
static void
headless_status_enablefield(int fieldidx, const char *nm, const char *fmt,
                            boolean enable)
{
    JBuf jb;
    jb_init(&jb);
    ...
    jb_end_obj(&jb);
    if (jb.ok)
        rpc_notify("status_enablefield", jb.buf);
    jb_free(&jb);
}
