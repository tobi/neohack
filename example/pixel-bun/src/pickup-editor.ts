...
export function validPickup(value: unknown): value is AutomaticPickup {
  ...
  return Object.keys(p).length === 5 && ["enabled", "arrows", "leaveCorpses", "leaveKnownCursed"].every(k => typeof p[k] === "boolean")
...
export function pickupEditor(initial: AutomaticPickup, rememberable = true) {
  ...
  const update = () => { summary.textContent = pickupSummary(draft); toggles.forEach(t => { t.input.checked = t.read(); }); };
  ...
  return {element:root, value:() => structuredClone(draft)};
}
