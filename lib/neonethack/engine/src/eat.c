...
/* neonethack modifications, 2026-09-03 through 2026-09-05: headless world
 * instrumentation. See CHANGES.neonethack.md; original NGPL notices retained. */
#include "hack.h"
...
/* Presentation-only de-duplication. The engine can defer do_reset_eat until
 * a later command after stop_occupation has already interrupted this meal.
 * Do not change victual/occupation state to report that boundary. */
static boolean headless_meal_interrupted;
staticfn void headless_meal_interruption(void);
staticfn void
headless_meal_interruption(void)
{
    if (!headless_meal_interrupted) {
        headless_meal_interrupted = TRUE;
        headless_action_result("eat", "interrupted");
    }
}

void
headless_meal_stopped(void)
{
    if (go.occupation == eatfood)
        headless_meal_interruption();
}
#endif
...
#ifdef HEADLESS_GRAPHICS
    headless_meal_interrupted = FALSE;
#endif
