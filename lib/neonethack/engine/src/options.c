...
/* Modified for neonethack 2026-09-05: profile 1 never reads user RC/options. */
#include "hack.h"
#include "tcap.h"
...
{   nhsym sym = 0;
    ...
    if (headless_runtime_epoch() >= 0) {
        char fixed_options[] = "!tutorial,time,number_pad:0";
        (void) parseoptions(fixed_options, TRUE, FALSE);
    } else
#endif
        rcfile();
