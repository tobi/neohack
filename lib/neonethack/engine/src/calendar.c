...
/* Modified for neonethack 2026-09-05: profile-1 calendar is fixed at the
 * recorded creation epoch, interpreted in UTC (never the host time zone). */
#include "hack.h"
...
getnow(void)
{
    ...
    if (headless_runtime_epoch() >= 0)
        return (time_t) headless_runtime_epoch();
#endif
...
static struct tm *
calendar_tm(time_t *date)
{
#ifdef HEADLESS_GRAPHICS
    if (headless_runtime_epoch() >= 0)
        return gmtime((LOCALTIME_type) date);
#endif
