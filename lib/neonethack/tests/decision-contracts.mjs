...
export function decisionContracts(name, fixture) {
  for (const kind of ['item', 'confirmation', 'choice']) test(`${name}: ${kind} rejection frames preserve the standing decision and exact old receipts`, async t => {
    ...
    const cancelled = await game.cancel(game.decision.id); assert.equal(game.decision, null);
    assert.equal(cancelled.outcome.status, 'cancelled');
    assert.equal(cancelled.outcome.turnsElapsed, 0);
    const narration = [...cancelled.observation.heard, ...cancelled.events.filter(e => e.type === 'message').map(e => e.text)];
    assert.ok(!narration.some(text => /Program in disorder|yn_function\(\) returned|report these messages/.test(text)), JSON.stringify(narration));
