...
export async function pickupEngine(t, wasm = false) {
  ...
  const source = original.replace('static char *\nhl_input(', `#include ${JSON.stringify(`${root}/tests/pickup-fixture.inc`)}\nstatic char *\nhl_input(`);
  ...
  const fixtureMain = main.replace(initial, '        { extern void pickup_fixture(void); pickup_fixture(); }\n' + initial);
  ...
  build.sourceHash = hash(source + fixtureMain + await readFile(`${root}/tests/pickup-fixture.inc`,'utf8'));
