...
test("WebMCP rolls back partial registration, supports unavailable browsers and reports uncertain transport", async () => {
  ...
  const registration = await registerWebMcp(
    ...
    {
      registerTool(tool) {
        if(tool.name === "session_create")execute = tool.execute;
      },
      unregisterTool() {},
    },
  );
  ...
  assert.match(failed.structuredContent.summary, /Do not resubmit session.create/);
  registration.dispose();
});
...
test('default WebMCP agent profile exposes navigation and owns exact input guards', async t => {
  ...
  assert.ok(created.observation.neighborhood); assert.equal(created.update, undefined);
  const waited = await call('game_wait', sid);
  assert.equal(waited.observation.turn, created.observation.turn + 1);
  assert.equal(requests.at(-1).params.expectedRevision, created.revision);
  assert.equal(requests.at(-1).params.requestId, waited.operationId);
  assert.equal(waited.requestId, undefined); assert.ok(waited.summary);
  const before = requests.length;
  assert.equal((await call('game_wait', {...sid, expectedRevision: 0})).error.code, 'invalidParams');
  assert.equal(requests.length, before);
  const cancelled = new AbortController(); cancelled.abort();
  assert.equal((await registered.get('game_wait').execute(sid, {signal: cancelled.signal})).isError, true);
  assert.equal(requests.length, before);
