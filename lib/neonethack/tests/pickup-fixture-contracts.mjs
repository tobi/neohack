...
import {pickupPerceptionContract} from './pickup-perception-contracts.mjs';
...
export async function pickupFixtureContracts(t, fixture) {
  await t.test('navigation stops on a real hunger transition before the next planned step',async t=>{
    const b=await fixture(t),game=await b.api.create({...hero,name:'NavigationHunger'});
    const origin=game.observation.you,to={x:origin.x+2,y:origin.y};
    assert.equal(game.observation.vitals.hunger,'not_hungry');
    const route=await game.route(to);assert.equal(route.distance,2);
    const result=await game.go({to});
    assert.equal(result.reason,'changed');assert.equal(result.actionsTaken,1);
    assert.equal(game.observation.vitals.hunger,'hungry');
    assert.deepEqual(game.observation.you,{x:route.steps[0].x,y:route.steps[0].y});
  });
  await t.test('pickup review publishes the moved hero consistently before an answer', t => pickupPerceptionContract(t, fixture));
