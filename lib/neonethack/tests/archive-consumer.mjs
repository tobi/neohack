...
for (const backend of ['native', 'wasm']) {
  ...
  try {
    ...
    // Installed public exports must register the complete catalog and reach the
    // actual C backend. Browser-native registration is tested separately.
    const registered = new Map();
    const registration = await registerWebMcp(api.transport, {
      registerTool(tool) { registered.set(tool.name, tool); },
      unregisterTool(name) { registered.delete(name); },
    });
    assert.equal(registration.supported, true);
    assert.equal(registration.toolCount, registered.size);
    assert.ok(registered.has('go') && !registered.has('game_move'));
    const described = await registered.get('help').execute({});
    assert.equal(described.isError, false);
    assert.ok(described.structuredContent.tools.some(tool=>tool.name==='go'));
...
try {
  await client.connect(new StdioClientTransport({ command: `${prefix}/bin/neonethack-mcp`,
    args: [paths.enginePath, paths.dataPath, resolve('mcp-worlds')],
    stderr: 'pipe',
  }));
  assert.ok((await client.listTools()).tools.length > 20);
  const call = async (name, args) => {
    const result = await client.callTool({ name: `${name}`, arguments: args });
    assert.equal(result.isError, false); assert.deepEqual(result.content, []);
    return result.structuredContent;
  };
  assert.ok((await call('help', {})).tools.some(tool=>tool.name==='go'));
  const game = await call("create", hero);
  const moved = await call("wait", { sessionId: game.sessionId });
  assert.equal(moved.observation.turn, game.observation.turn + 1);
  await call("suspend", { sessionId: game.sessionId });
} finally { await client.close(); }
console.log('Installed npm native/WASM gameplay, consent/resume/receipts, WebMCP exports and official SDK MCP bin gameplay passed');
