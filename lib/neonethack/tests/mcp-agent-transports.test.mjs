...
for(const mode of ['stdio','http'])test(mode+' shared WASM navigation, guards and decisions',async t=>{
  ...
  const receipt=await call('receipt',{...sid,operationId:wait.operationId});assert.deepEqual(perceivedFrame(receipt.receipt.observation),perceivedFrame(wait.observation));assert.equal(receipt.historical,true);
  ...
  // Recorded seed-42 regression: explore used to walk toward the second
  // room's closed door, then fail its malformed door substep and hide the
  // committed walking behind "no operation was sent". Routes now displace the
  // kitten instead of corking on it, so the door leg comes after the eastern
  // rooms are exhausted.
  ...
  for(let i=0;i<23;i++){
    ...
    if(frame.outcome.effects.includes('swappedPlaces'))swaps++;
  }
  assert.ok(swaps>0,'the seeded walk displaces the kitten at least once');
  assert.equal(frame.observation.turn,76);
  ...
  assert.equal(opened.navigation.actionsTaken,15,'fourteen walking steps and one door attempt');
  assert.equal(opened.navigation.turnsElapsed,15);
  assert.equal(opened.observation.turn,91);
  assert.deepEqual(opened.observation.you,{x:41,y:7});
  const doorRow=opened.map.text.split('\n').find(r=>r.startsWith('  7 '));
  // NetHack's default symset draws an open door as the swung door (- or |), never an invented glyph.
  const doorMark=doorRow[4+40-opened.map.bounds.x[0]];
  assert.ok(doorMark==='-'||doorMark==='|','the opened door is rendered with the engine mark');assert.ok(opened.map.legend[doorMark].includes('open door'));
  const doorCell=(await call("syncState",run)).observation.world.find(c=>c.x===40&&c.y===7);
  assert.equal(doorCell?.terrain.type,'openDoor');assert.equal(doorCell?.terrain.mark,doorMark,'JSON terrain.mark is the same engine character the map prints');
  assert.equal((await call("syncState",run)).observation.turn,91);
