...
test('browser controlled marker and spell chains expose each genuine item, text and target choice', {timeout:60_000}, async t=>{
  ...
  const {readFile}=await import('node:fs/promises');const {fileURLToPath}=await import('node:url');const {dirname,join}=await import('node:path');
  const options=await scenarioEngine(t,true,'manual-fixture.inc','manual_fixture');
  const directory=dirname(fileURLToPath(options.workerUrl));const {page,errors}=await fixture(t);
  /* Serve a distinct, verified fixture package at this test origin. No release
   * files, browser sandbox flags or package-integrity checks are changed. */
  await page.context().route('**/__manual_fixture__/*',async route=>{
    const name=new URL(route.request().url()).pathname.split('/').at(-1);assert.match(name,/^[\w.-]+$/);
    await route.fulfill({status:200,contentType:name.endsWith('.mjs')?'text/javascript':name.endsWith('.json')?'application/json':name.endsWith('.wasm')?'application/wasm':'application/octet-stream',body:await readFile(join(directory,name))});
  });
  ...
  await page.evaluate(async()=>{
    ...
    await element.api.close();element.api=await createWasm({workerUrl:new URL('/__manual_fixture__/core-worker.mjs',location.href)});
    element.game=await element.api.create({name:'BrowserManual',seed:42,role:'wizard',race:'human',gender:'female',align:'neutral'});element.show(element.game.state);element.renderControls();
  });
