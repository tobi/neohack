...
test('human replacement of a question cannot retarget a registered WebMCP answer', {timeout:60_000}, async t=>{
  ...
  await page.evaluate(async sid=>{
    ...
    await globalThis.bindingTools.get('session_observe').execute({sessionId:sid});
  },created.sessionId);
  await page.getByRole('button',{name:'Cancel choice',exact:true}).click();
  await waitFrame(page,`${frameExpression}.decision === null`);
  await page.getByRole('button',{name:'Pray',exact:true}).click();
  ...
  const result=await page.evaluate(async({sid,oldId})=>{
    ...
    await tools.get('session_observe').execute({sessionId:sid});
    const stale=await tools.get('decision_answer').execute({sessionId:sid,decisionId:oldId,answer:{kind:'confirmation',confirm:true}});
    const current=await tools.get('session_observe').execute({sessionId:sid});
...
test('browser WebMCP leads with terminal facts after explicit quit and keeps disconnected close resumable', {timeout:60_000}, async t=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    try{
      const created=await call('session_create',{seed:42,role:'valkyrie'}),sid={sessionId:created.sessionId};
      const closed=await call('session_close',sid),resumed=await call('session_resume',sid);
      const question=await call('game_quit',sid);
      const terminal=await call('decision_answer',{...sid,decisionId:question.decision.id,answer:{kind:'confirmation',confirm:true}});
  ...
  for(const frame of [result.terminal,result.receipt,result.observed,result.navigation]){
  ...
  assert.deepEqual(result.receipt.outcome,result.terminal.outcome);
