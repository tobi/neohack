...
test('compressed browser walking has bounded writes and retains old receipts and warnings after abrupt reload', { timeout: 60_000 }, async t => {
  ...
  await page.evaluate(async () => {
    ...
    globalThis.walkGame = await globalThis.walkApi.create({ name: 'Walking', seed: 42, role: 'valkyrie', race: 'dwarf', gender: 'female', align: 'lawful' });
  });
  ...
  await worker.evaluate(() => {
    ...
    IDBObjectStore.prototype.put = function (value, ...args) {
      ...
      globalThis.written.bytes += bytes; globalThis.written.maximum = Math.max(globalThis.written.maximum, bytes);
      if (this.name === 'blocks') globalThis.written.blocks++;
  ...
  const saved = await page.evaluate(async () => {
    ...
    for (let i = 0; i < 80; i++) {
      const o = globalThis.walkGame.observation, you = o.you;
      ...
      const request = { version: 1, method: 'game.move', params: { sessionId: globalThis.walkGame.id, requestId: `walking-${i}`, expectedRevision: globalThis.walkGame.state.revision, direction } };
      const started = performance.now(), receipt = await globalThis.walkApi.transport.send(request);
      ...
      await globalThis.walkGame.observe();
    }
    const prayer = await globalThis.walkGame.pray();
    return { receipts, prayer, times };
  });
  const writes = await worker.evaluate(() => globalThis.written);
...
test('aborted browser pre-input transaction sends no engine input and cannot be retried on close', async t => {
  ...
  const id = await page.evaluate(async () => {
    ...
    globalThis.abortGame = await globalThis.abortApi.create({ name: 'Abort', seed: 42, role: 'valkyrie', race: 'dwarf', gender: 'female', align: 'lawful' });
    return globalThis.abortGame.id;
  });
  ...
  await worker.evaluate(() => {
    ...
    const write = globalThis.__nnhHost.write; globalThis.__nnhHost.write = (...args) => { globalThis.engineWrites++; return write(...args); };
    const put = IDBObjectStore.prototype.put;
    IDBObjectStore.prototype.put = function(...args) {
      const result = put.apply(this, args);
      if (!globalThis.aborts && this.name === 'files') { globalThis.aborts++; this.transaction.abort(); }
      return result;
    };
  });
  const response = await page.evaluate(async () => {
    const response = await globalThis.abortApi.transport.send({ version: 1, method: 'game.move', params: { sessionId: globalThis.abortGame.id, requestId: 'aborted-move', expectedRevision: globalThis.abortGame.state.revision, direction: 'east' } });
    return response;
  });
  assert.ok(response.error || response.storage?.status === 'degraded');
  assert.deepEqual(await worker.evaluate(() => ({ engineWrites: globalThis.engineWrites, aborts: globalThis.aborts })), { engineWrites: 0, aborts: 1 });
  assert.equal(await storedFile(page, 'abort-blocks', `${base}/input.log.jsonl`), before);
  await page.evaluate(() => globalThis.abortApi.close());
  assert.equal(await storedFile(page, 'abort-blocks', `${base}/input.log.jsonl`), before);
});
...
test('human replacement of a question cannot retarget a registered WebMCP answer', {timeout:60_000}, async t=>{
  ...
  await page.evaluate(async sid=>{
    ...
    await globalThis.bindingTools.get("syncState").execute({sessionId:sid});
  },created.sessionId);
  await page.getByRole('button',{name:'Cancel choice',exact:true}).click();
  await waitFrame(page,`${frameExpression}.decision === null`);
  await page.getByRole('button',{name:'Pray',exact:true}).click();
  ...
  const result=await page.evaluate(async({sid,oldId})=>{
    ...
    await tools.get("syncState").execute({sessionId:sid});
    const stale=await tools.get("answer").execute({sessionId:sid,decisionId:oldId,value: true});
    const current=await tools.get("syncState").execute({sessionId:sid});
...
test('browser WebMCP leads with terminal facts after explicit quit and keeps disconnected close resumable', {timeout:60_000}, async t=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    try{
      ...
      const observed=await call("syncState",sid);
