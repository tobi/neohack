...
for (const fault of ['lost', 'slow']) test(`browser WebMCP ${fault} create reply is recovered without another creation`, { timeout: 60000 }, async t => {
  ...
  const result = await page.evaluate(async fault => {
    ...
    const { CompactObservationReader } = await import('/lib/neonethack/dist/mcp/compact.js');
    ...
    const invocation = tools.get('session_create').execute({ name: 'Fresh', seed: 42, role: 'valkyrie', race: 'dwarf', gender: 'female', align: 'lawful' });
    ...
    const observed = await tools.get('session_observe').execute({ sessionId });
    reader.apply(observed.structuredContent);
    let settled = false; invocation.then(() => { settled = true; });
    await Promise.resolve();
    const pendingBeforeRelease = !settled;
    release();
    const recovered = await invocation;
    const recoveredFrame = recovered.isError ? null : reader.apply(recovered.structuredContent);
  ...
  if (fault === 'lost') {
    ...
    assert.match(result.recovered.content[0].text, /Do not resubmit session.create/);
  } else {
    assert.equal(result.pendingBeforeRelease, true);
    const expected = structuredClone(result.ownedCreation);
    delete expected.observation.neighborhood; // MCP omits offers except on explicit observe.
    assert.deepEqual(result.recoveredFrame, expected);
  }
