...
for (const fault of ['lost', 'slow']) test(`browser WebMCP ${fault} create reply is recovered without another creation`, { timeout: 60000 }, async t => {
  ...
  const result = await page.evaluate(async fault => {
    ...
    const observed = await tools.get("syncState").execute({ sessionId });
  ...
  if (fault === 'lost') {
    ...
    assert.deepEqual(result.recoveredFrame.presentation.omitted,['observation.world','observation.neighborhood','observation.heard','observation.knowledge']);
    assert.equal(result.recoveredFrame.presentation.fullObservation,"syncState");
    assert.equal(typeof result.recoveredFrame.map.text,'string');
    for(const key of ['neighborhood','world','heard','knowledge'])delete expected.observation[key];
