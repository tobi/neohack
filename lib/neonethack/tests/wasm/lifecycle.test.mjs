...
for (const fault of ['lost', 'slow']) test(`browser WebMCP ${fault} create reply is recovered without another creation`, { timeout: 60000 }, async t => {
  ...
  if (fault === 'lost') {
    ...
    assert.match(result.recovered.structuredContent.summary, /Do not resubmit session.create/);
  } else {
    ...
    assert.equal(result.recoveredFrame.presentation.fullObservation,'session_observe');
