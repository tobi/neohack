...
import { containerContracts } from '../container-contracts.mjs';
import { pickupSettingsContracts } from '../pickup-settings-contracts.mjs';
...
agentContracts(test, 'WASM', fixture);
loreContracts(test,'WASM',fixture);
countedContracts(test,'WASM',fixture);
...
pickupSettingsContracts('WASM', restartable);
test('WASM: controlled real pickup and container engine scenarios', async t => {
  const options = await pickupEngine(t, true);
  await pickupFixtureContracts(t, async t => {
...
const { identityContracts } = await import('../identity-contracts.mjs');
identityContracts(test, 'WASM', fixture);

containerContracts('WASM', restartable);

const { routeContracts } = await import('../route-contracts.mjs');
routeContracts(test, 'WASM', fixture);
