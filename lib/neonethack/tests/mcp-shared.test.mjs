...
test('shared executor captures queued revisions, preserves uncertainty and recovers an exact accepted input',async t=>{
  ...
  const recovered=await call('recover',sid);assert.equal(recovered.operationId,lost.operationId);assert.equal(recovered.observation.turn,a.observation.turn+1);
  const receipt=await call('receipt',{...sid,operationId:lost.operationId});assert.deepEqual(receipt.observation,recovered.observation);assert.deepEqual(receipt.outcome,recovered.outcome);
});
