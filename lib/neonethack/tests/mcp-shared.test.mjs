...
for(const mode of ['stdio','http'])test(`${mode} and WebMCP execute identical schemas and real WASM journey replies`,async t=>{
  ...
  let sa=a,sb=b;
  for(const [name,params] of [['route',{to:a.observation.you}],['inspect',{}],['go',{to:a.observation.you}],['search',{turns:5}],['pray',{}],['cancel',null],['explore',{maxActions:4}],['syncState',{}]]){
    ...
    if(sb.map){
      ...
      assert.equal(result.content[1].text,`Perceived map (exact feature coordinates: map.positions):\n~~~~text\n${sb.map.text}\n~~~~`);
    }else assert.equal(result.content.length,1);
...
test('shared executor captures queued revisions, preserves uncertainty and recovers an exact accepted input',async t=>{
  ...
  const recovered=await call('syncState',sid);assert.equal(recovered.verification.operationId,lost.operationId);assert.equal(recovered.observation.turn,a.observation.turn+1);
