...
export function identityContracts(test, label, fixture) {
  test(`${label}: caller attribution leaves game identity and resumed perception unchanged`, {timeout:30000}, async t => {
    const {api} = await fixture(t);
    const identity={name:'Attribution',role:'valkyrie',seed:42};
    const plain=await api.create(identity), expected=plain.observation;
    await plain.close();
    const labelled=await api.create({...identity,harness_name:'Pi',model_name:'Muse Spark'});
    assert.deepEqual(labelled.observation,expected);
    const id=labelled.id;await labelled.close();
    assert.deepEqual((await api.resume(id)).observation,expected);
    for(const labels of [{harness_name:'x'.repeat(121)},{model_name:'bad\nlabel'}])
      await assert.rejects(api.create({...identity,...labels}), /attribution|control|120|field|identity|schema/i);
  });
