...
for(const mode of ['stdio','http','typescript'])test(`${mode}: decision identities survive resume and reject stale confirmations, cancellations and named transfers`,async t=>{
  ...
  const current=await B("observe",sid);assert.equal(current.ended,false);assert.equal(current.observation.turn,quit.observation.turn);assert.deepEqual(current.decision,quit.decision);
  const declined=await B("answer",{...sid,decisionId:quit.decision.id,value: false});assert.equal(declined.decision,null);
  const retry=await A("recover",sid);assert.equal(retry.operationId,declined.operationId);assert.equal(retry.historical,true);assert.equal(retry.observation.turn,declined.observation.turn);
  ...
  assert.equal((await B("observe",sid)).observation.turn,declined.observation.turn);
  await A("suspend",sid);
  await B("resume",sid);
  ...
  const still=await B("observe",bagRun);assert.equal(still.observation.turn,takeMenu.observation.turn);assert.deepEqual(still.decision,takeMenu.decision);
  ...
  assert.equal((await A("recover",bagRun)).operationId,taken.operationId);
