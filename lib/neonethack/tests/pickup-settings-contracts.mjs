...
export function pickupSettingsContracts(name, fixture) {
  test(`${name}: automatic pickup settings are validated, zero-turn, journaled and receipt-idempotent`, async t => {
    ...
    const configured = {...defaults, enabled:false, itemTypes:['food','potions'], lootPatterns:['RATION', 'a+b.[x]', 'named \"friend\"'], ignorePatterns:['corpse']};
    ...
    assert.deepEqual(game.observation.automaticPickup, configured);
    for(const patterns of [[''], ['   '], ['x\ny'], ['a'.repeat(65)], Array(17).fill('arrow'), [12], 'arrow'])
      await assert.rejects(game.configurePickup({...defaults,lootPatterns:patterns}));
    await game.configurePickup(defaults);
    assert.deepEqual(game.observation.automaticPickup, defaults, 'replacement clears omitted patterns');
    await game.pray();
  ...
  for (const [description, settings, category, collected] of [
    ...
    ['loot substrings collect with categories and arrows off', {...defaults,itemTypes:[],arrows:false,lootPatterns:['ArRoW']}, 'arrow', true],
    ['ignore substrings override category and arrow inclusion', {...defaults,itemTypes:['weapons'],lootPatterns:['arrow'],ignorePatterns:['ARROW']}, 'arrow', false],
    ['patterns are literal, not regular expressions', {...defaults,itemTypes:[],arrows:false,lootPatterns:['.*']}, 'arrow', false],
