...
export function positionContracts(label, fixture) {
test(`${label}: food detection exposes a durable position decision instead of a resting command`, async t => {
  const backend = await fixture(t);
  const {api, transport} = backend;
  ...
  const resumed = await backend.restart(game);
  game = resumed.game;
