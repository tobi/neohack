...
for(const mode of ['stdio','http','typescript','webmcp'])test(`${mode}: real terminal input, postmortem navigation/observe and disconnected close summaries`,{timeout:30000},async t=>{
  ...
  assert.deepEqual(receipt.receipt.outcome,death.outcome);assert.deepEqual(receipt.receipt.end,death.end);
  assert.equal(receipt.operationId,death.operationId);assert.equal(receipt.receipt.summary,death.summary);
...
for(const mode of ['stdio','http','typescript','webmcp'])test(`${mode}: death during a real navigation action leads with cause and preserves its receipt`,{timeout:30000},async t=>{
  ...
  const rows=start.map.text.split('\n'),row=rows.find(r=>r.startsWith(String(next.y).padStart(3)+' '));
  assert.ok(row&&row[4+next.x-start.map.bounds.x[0]]!==' ','destination is perceived at creation (rendered on the map)');
  ...
  for(let i=0;i<100&&!state.ended;i++){
    ...
    state=await call('go',{...sid,direction:at.x===origin.x&&at.y===origin.y?'north':'south',maxActions:1});
  ...
  assert.deepEqual(receipt.receipt.outcome,state.outcome);assert.deepEqual(receipt.receipt.end,state.end);
  assert.ok(receipt.receipt.summary.startsWith('Run ended (death): killed by a fox, while fainted from lack of food.'));
});
