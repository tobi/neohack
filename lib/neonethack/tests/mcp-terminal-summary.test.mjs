...
test('terminal summary fixtures agree in full and compact C/TS presentations without changing facts',async t=>{
  ...
  for(const compact of [false,true]){
    const executable=process.env.NEONETHACK_AGENT_TEST_EXECUTABLE??`${root}/build/native/neonethack-mcp-agent-test`;
    const native=execFileSync(executable,[compact?'present-compact':'present'],{
    ...
    for(const [i,frame] of frames.entries()){
      ...
      assert.deepEqual(native[i],ts,`C/TS presentation parity for fixture ${i}, compact=${compact}`);
    ...
    assert.equal(native[0].summary,'Run ended (death): killed by a frost giant. rest: completed; 0 turns elapsed.');
    assert.equal(native[1].summary,'Run ended (death): killed by a frost giant. Navigation: ended; 1 actions, 0 turns elapsed.');
    assert.equal(native[2].summary,'Run ended (death): killed by a frost giant. get_state: completed; 0 turns elapsed.');
  }
});
...
async function client(t,mode){
  ...
  if(mode==='stdio'){
    ...
    await sdk.connect(new StdioClientTransport({command:mcpExecutable,args:Object.values(options),stderr:'pipe'}));
...
for(const mode of ['stdio','http','typescript','webmcp'])test(`${mode}: real terminal input, postmortem navigation/observe and disconnected close summaries`,{timeout:30000},async t=>{
  ...
  const start=await call('session_create',{seed:4386,name:'Lifesaving',role:'tourist',race:'human',gender:'female',align:'neutral'});
  const sid={sessionId:start.sessionId};assert.ok(sid.sessionId,JSON.stringify(start));
  const closed=await call('session_close',sid);
  ...
  const resumed=await call('session_resume',sid);
  ...
  await call('game_pickup',{...sid,item:{id:amulet.id}});
  const waited=await call('game_wait',sid);
  const food=waited.observation.inventory.find(item=>item.category==='food'&&item.label.includes('food ration'));assert.ok(food);
  await call('game_eat',{...sid,item:{id:food.id}});
  const warning=await call('game_eat',{...sid,item:{id:food.id}});
  assert.equal(warning.decision.kind,'confirmation');assert.equal(warning.decision.about,'Continue eating?');
  const death=await call('decision_answer',{...sid,decisionId:warning.decision.id,answer:{kind:'confirmation',confirm:true}});
  ...
  const observed=await call('session_observe',sid);
...
for(const mode of ['stdio','http','typescript','webmcp'])test(`${mode}: death during a real navigation action leads with cause and preserves its receipt`,{timeout:30000},async t=>{
  const call=await client(t,mode);
  const start=await call('session_create',{seed:42,name:'Terminal',role:'tourist',race:'human',gender:'female',align:'neutral'});
  ...
  for(let i=0;i<40&&state.observation.vitals.hunger!=='fainting';i++){
    state=await call('game_rest',{...sid,turns:50});
