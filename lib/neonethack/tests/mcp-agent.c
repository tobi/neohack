...
int main(int argc, char **argv)
{
    ...
    char *clarified = mcp_agent_execute(NULL,&choices,"decision.answer",(mj_val){"{\"sessionId\":\"abcdefghijklmnop\",\"decisionId\":\"question\",\"value\":[\"same\"]}"},4,"unused");
    assert(clarified && strstr(clarified,"ambiguousName") && !choices.pending && !choices.last); free(clarified);
    clarified = mcp_agent_execute(NULL,&choices,"decision.answer",(mj_val){"{\"sessionId\":\"abcdefghijklmnop\",\"decisionId\":\"question\",\"value\":[\"missing\"]}"},4,"unused");
    assert(clarified && strstr(clarified,"unknownName") && !choices.pending); free(clarified);
    mcp_agent_clear(&choices);
    mj_val go = mcp_agent_method("go"), answer = mcp_agent_method("answer");
    ...
    const char *args = "{\"sessionId\":\"abcdefghijklmnop\",\"decisionId\":\"standing-context\",\"value\":false}";
    assert(mcp_agent_valid(answer,(mj_val){args}));
    assert(!mcp_agent_valid(answer,(mj_val){"{\"sessionId\":\"abcdefghijklmnop\",\"value\":false}"}));
    char *normalized = mcp_agent_arguments((mj_val){args},(mj_val){"\"confirmation\""});
    assert(normalized);
    char *request = mcp_agent_request("decision.answer",(mj_val){normalized},7,"unique-operation");free(normalized);
