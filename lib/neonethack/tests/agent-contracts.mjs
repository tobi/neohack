...
export function agentContracts(test,label,fixture) {
  test(`${label}: named spell choices resolve the standing question without spending input on an unknown alias`,async t=>{
    ...
    const unknown=await agent.call('decision_answer',{...sid,answer:{kind:'choice',choose:['not-a-spell']}});
    ...
    assert.ok(chosen);
    const result=await agent.call('decision_answer',{...sid,answer:{kind:'choice',choose:[chosen.name]}});
  ...
  test(`${label}: agent profile owns guards, names its tools and keeps decisions explicit`,async t=>{
    ...
    assert.ok(agent.tools.every(t=>!t.inputSchema.properties.requestId && !t.inputSchema.properties.expectedRevision && !t.inputSchema.properties.decisionId));
    ...
    const cancel=await agent.call('decision_cancel',sid);assert.equal(cancel.decision,null);
    const historical=await agent.call('receipt',{...sid,operationId:wait.operationId});
    assert.equal(historical.historical,true);assert.deepEqual(historical.observation,wait.observation);
  ...
  test(`${label}: lost input responses keep the exact operation and retry its receipt once`,async t=>{
    ...
    const recovered=await agent.call('retry',sid);assert.deepEqual(recovered.observation,original.observation);assert.equal(recovered.operationId,original.requestId);
  ...
  test(`${label}: an explicit uncertain result blocks new input even after observation`,async t=>{
    ...
    assert.deepEqual(retry.observation,saved.observation);
  ...
  test(`${label}: a lost outer reply can be recovered without rewinding a newer human observation`,async t=>{
    ...
    assert.deepEqual(recovered.observation,deliveredToBridge.observation);
