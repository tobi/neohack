...
export function agentContracts(test,label,fixture) {
  ...
  test(`${label}: navigation recovery distinguishes the pending input from the last confirmed receipt`,async t=>{
    ...
    const recovered=await agent.call("observe",sid);
  ...
  test(`${label}: resumed named container decisions reject an old opposite transfer`,async t=>{
    ...
    assert.equal((await agent.call("observe",sid)).revision,taken.revision);
  });
  ...
  test(`${label}: observe verifies a lost input response without dispatching gameplay again`,async t=>{
    ...
    const recovered=await agent.call("observe",sid);assert.deepEqual(perceivedFrame(recovered.observation),perceivedFrame(original.observation));assert.equal(recovered.verification.operationId,original.requestId);
    assert.equal(dispatched,1);assert.equal((await agent.call("observe",sid)).verification,undefined);assert.equal(dispatched,1);
  ...
  test(`${label}: verification of a lost decision reply returns a newer participant's actual standing question`,async t=>{
    ...
    const recovered=await agent.call("observe",sid);assert.equal(recovered.verification.operationId,original.requestId);
  ...
  for(const obscuration of ['error','unknown'])test(`${label}: an explicit ${obscuration} result is settled only by its exact verified receipt`,async t=>{
    ...
    assert.deepEqual(uncertain.next,{tool:'observe',arguments:sid});
    assert.equal((await agent.call("wait",sid)).error.code,'uncertainExecution');
    assert.equal(inputs,1);
    const retry=await agent.call("observe",sid);
  ...
  test(`${label}: after a lost outer reply observe never guesses an old receipt or rewinds human movement`,async t=>{
  ...
  test(`${label}: loss before dispatch cannot turn observe into the original action`,async t=>{
    ...
    for(let i=0;i<2;i++){
      const current=await agent.call('observe',sid);assert.equal(current.error.code,'uncertainExecution');
  ...
  test(`${label}: a verified receipt without a fresh scene cannot clear input uncertainty`,async t=>{
    ...
    const start=await agent.call('create',identity),sid={sessionId:start.sessionId};
    await agent.call('search',{...sid,turns:1});
    assert.equal((await agent.call('observe',sid)).error.code,'uncertainExecution');
    assert.equal((await agent.call('search',{...sid,turns:1})).error.code,'uncertainExecution');
    failObserve=false;
  ...
  test(`${label}: known navigation stops need no recovery and historical questions are not executable`,async t=>{
    ...
    const current=await agent.call('observe',sid);assert.deepEqual(current.observation.you,moved.observation.you);
    ...
    assert.equal((await agent.call('observe',sid)).decision,null);
  });
}
