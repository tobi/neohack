...
test('native agent presentation preserves actual engine frames and matches WebMCP creature references', async t => {
  ...
  frames.push(query);
  frames.push(await game.actions({direction:"north"}));
...
test('native agent-owned guard construction reaches real engine decisions and exact receipts', async t => {
  ...
  const build=async (name,args,revision,operationId)=>{
    const child=spawn(agentExecutable,['request'],{stdio:['pipe','pipe','pipe']});
    let output='',errors='';child.stdout.on('data',b=>output+=b);child.stderr.on('data',b=>errors+=b);
    child.stdin.end(JSON.stringify({name,args,revision,operationId})+'\n');
    const [code]=await once(child,'exit');assert.equal(code,0,errors);return JSON.parse(output);
  };
  const sid={sessionId:game.id};
  const wait=await build("wait",sid,game.state.revision,'native-agent-once');
  const first=await transport.send(wait);assert.equal(first.observation.turn,game.observation.turn+1);
  const stale=await transport.send(await build("wait",sid,game.state.revision,'native-agent-stale'));
  ...
  const ask=await transport.send(await build("eat",sid,first.revision,'native-agent-question'));
  assert.equal(ask.decision.kind,'item');
  const cancel=await build("cancel",{...sid,decisionId:ask.decision.id},ask.revision,'native-agent-cancel');
...
test('native agent executor preserves queued revisions and binds actual standing decisions', async t => {
  ...
  const lore=await call("lookup",{...sid,name:'floating eye'},created.revision);
  assert.equal(lore.kind,'lore');assert.equal(lore.found,true);assert.ok(lore.lines.length);
  const wait=await call("wait",sid,created.revision,'native-owned-wait');
  assert.equal(wait.observation.turn,created.observation.turn+1);
  const stale=await call("wait",sid,created.revision,'native-queued');
  assert.equal(stale.error.code,'staleRevision');
  const asked=await call("eat",sid,wait.revision,'native-owned-eat');
  assert.equal(asked.decision.kind,'item');
  const cancel=await call("cancel",{...sid,decisionId:asked.decision.id},asked.revision,'native-owned-cancel');
  ...
  const after=await call("wait",sid,cancel.revision,'native-after-receipt');
  ...
  const mage=await call("create",{name:'Mage',seed:42,role:'wizard',race:'human',gender:'female',align:'neutral'});
  const wizard={sessionId:mage.sessionId};
  const spells=await call("cast",wizard,mage.revision,'spell-menu');
  assert.equal(spells.decision.kind,'choice');
  const unknown=await call("answer",{...wizard,decisionId:spells.decision.id,value: ['missing']},spells.revision,'unknown-spell');
  assert.equal(unknown.clarification.reason,'unknownName');
  const first=spells.decision.options[0];assert.ok(first.name);
  const selected=await call("answer",{...wizard,decisionId:spells.decision.id,value: [first.name]},spells.revision,'named-spell');
...
test('native navigation executor agrees with WebMCP bounded exploration and direct movement', async t => {
  ...
  const options={...identity,seed:7};const native=await call("create",options),browser=await js.call("create",options);
  const compare=(a,b)=>{
...
test('native navigation preserves committed progress when later supervisor retention fails',async t=>{
  ...
  const created=await call("create",{role:'valkyrie',seed:42});
  ...
  const duplicate=await call("recover");
...
test('compact native and WebMCP presentation preserve real decisions and perceived layers', async t => {
  ...
  for(let i=0;i<frames.length;i++){
    ...
    assert.equal(actual.presentation.fullObservation,"observe");
...
test('native MCP navigation stops at a real hunger transition before its next route step',async t=>{
  ...
  const created=await call("create",{name:'NavigationHunger',role:'valkyrie',race:'human',gender:'female',align:'lawful',seed:42});
  ...
  const route=await call("route",{...sid,to});assert.equal(route.distance,2);
