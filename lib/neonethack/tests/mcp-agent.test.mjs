...
test('native agent-owned guard construction reaches real engine decisions and exact receipts', async t => {
  ...
  const cancel=await build('decision_cancel',{...sid,decisionId:ask.decision.id},ask.revision,'native-agent-cancel');
...
test('native agent executor preserves queued revisions and binds actual standing decisions', async t => {
  ...
  const cancel=await call('decision_cancel',{...sid,decisionId:asked.decision.id},asked.revision,'native-owned-cancel');
  ...
  const unknown=await call('decision_answer',{...wizard,decisionId:spells.decision.id,answer:{kind:'choice',choose:['missing']}},spells.revision,'unknown-spell');
  assert.equal(unknown.clarification.reason,'unknownName');
  const first=spells.decision.options[0];assert.ok(first.name);
  const selected=await call('decision_answer',{...wizard,decisionId:spells.decision.id,answer:{kind:'choice',choose:[first.name]}},spells.revision,'named-spell');
