...
test('C official MCP SDK stdio roundtrip exposes strict tools and structured results', { timeout: 30_000 }, async t => {
  ...
  const invoke = async (name, args) => {
    ...
    assert.equal(typeof summary,'string');assert.ok(validateResponse(low),JSON.stringify(validateResponse.errors));
    return result;
  };
  ...
  assert.deepEqual((await invoke('receipt', {...prayerArgs,operationId:prayer.operationId})).structuredContent.observation,prayer.observation);
...
test('C stdio MCP framing, notifications and complete navigation observations', {timeout: 30000}, async t => {
  ...
  assert.deepEqual(observed.observation,state.observation,'complete agent observations need no delta reconstruction');
