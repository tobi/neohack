...
export async function startMcpHttp({ enginePath, dataPath, sessionsPath }) {
  const probe = createServer();
  probe.listen(0, '127.0.0.1');
  await once(probe, 'listening');
  const port = probe.address().port;
  await new Promise(resolve => probe.close(resolve));
  const process = spawn(mcpExecutable, ['--http', String(port), enginePath, dataPath, sessionsPath], { stdio: 'pipe' });
