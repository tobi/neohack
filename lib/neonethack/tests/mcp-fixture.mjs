...
export const mcpExecutable = process.env.NEONETHACK_MCP_TEST_EXECUTABLE ?? `${root}/dist/mcp/cli.js`;
export const HTTP_PROTOCOL_VERSION = '2026-07-28';
export async function startMcpHttp({ sessionsPath, runtimePath, mcpCommand = mcpExecutable, env }) {
  const probe = createServer();
  probe.listen(0, '127.0.0.1');
  await once(probe, 'listening');
  const port = probe.address().port;
  await new Promise(resolve => probe.close(resolve));
  const process = spawn(mcpCommand, ['--http', String(port), '--sessions', sessionsPath, ...(runtimePath?['--runtime',runtimePath]:[])], { stdio: 'pipe', env });
  ...
  await Promise.race([ready, exited.then(([code, signal]) => { throw Error(`MCP exited before listening: ${code}/${signal}: ${stderr}`); })]);
  let closing;
