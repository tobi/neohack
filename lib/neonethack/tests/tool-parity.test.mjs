...
import { compareTools, checkTools, operationCoverage } from '../scripts/check-tools.mjs';
...
test('operation coverage allows different vocabularies but rejects gaps and invented operations', () => {
  const operations = [{name:'game.move'}, {name:'game.attack'}];
  assert.deepEqual(operationCoverage(operations, [{name:'go',covers:['game.move']}, {name:'attack',covers:['game.attack']}]), []);
  assert.deepEqual(operationCoverage(operations, [{name:'go',covers:['game.move','game.teleport']}]), [
    'go: unknown operation game.teleport', 'uncovered operation: game.attack',
  ]);
  assert.deepEqual(operationCoverage(operations, [{name:'go',covers:['game.move']}], {'game.attack':'deliberately low-level only'}), []);
  assert.deepEqual(operationCoverage(operations, [{name:'go',covers:['game.move']}], {'game.move':'', 'game.fly':'x'}), [
    'omitted operation is also covered: game.move', 'omitted without a reason: game.move', 'omitted: unknown operation game.fly', 'uncovered operation: game.attack',
  ]);
});
