...
export function routeContracts(test, label, fixture) {
  test(`${label}: routes displace a tame ally and noRoute names unknown and disconnected destinations`,async t=>{
    ...
    const onto=await game.route({x:pet.x,y:pet.y});
    assert.equal(onto.distance,1,'an adjacent tame ally is a one-step displacement, not a blocked target');
    assert.equal(onto.why,undefined);assert.equal(onto.steps.length,1);
    assert.equal(onto.steps[0].x,pet.x);assert.equal(onto.steps[0].y,pet.y);
    const swapped=await game.go({to:{x:pet.x,y:pet.y}});
    assert.equal(swapped.reason,'arrived');assert.equal(swapped.actionsTaken,1);assert.equal(swapped.turnsElapsed,1);
    assert.deepEqual(game.observation.you,{x:pet.x,y:pet.y});
    assert.ok(swapped.snapshot.outcome.effects.includes('swappedPlaces'),JSON.stringify(swapped.snapshot.outcome));
  ...
  test(`${label}: explicitly bounded multi-frontier exploration matches separate deliberate legs`,async t=>{
    ...
    for(const game of [single,batch]) {
      ...
      assert.equal(game.observation.turn,51,'seeded fixture walks past its companion instead of corking on it');
    }
  ...
  test(`${label}: go direction is ordinary movement and attack remains a distinct engine action`, async t => {
    ...
    const direction=to.actions.find(a=>a.method==='game.move').arguments.direction;
    const moved=await hero.go({direction});
