...
export function gameplayContracts(name, fixture) {
  test(`${name}: first-seen items and creatures have visible terrain beneath them`, async t => {
    const b = await fixture(t);
    let game = await b.api.create(valkyrie);
    const initial = game.observation;
    const items = initial.world.filter(cell => cell.visible && cell.objects?.length);
    const creatures = initial.world.filter(cell => cell.visible && cell.occupant?.kind === 'ally');
    assert.ok(items.length, 'seed must include an actual first-seen floor item');
    assert.ok(creatures.length, 'seed must include an actual visible pet');
    for (const cell of [...items, ...creatures]) assert.equal(cell.terrain.type, 'floor');
    assert.ok(initial.world.filter(cell => !['dark', 'unknown'].includes(cell.terrain.type)).length < 200,
      'supplying visible backgrounds must not expose the unexplored level');
    ...
    for (const cell of items) {
      const remembered = game.observation.world.find(other => other.x === cell.x && other.y === cell.y);
      assert.equal(remembered.visible, false);
      assert.equal(remembered.terrain.type, 'floor', 'unseen item retains its legitimately perceived base');
    }
