...
const validate = new Ajv({ strict: false }).compile(compactResponseSchema);
const meta = {
...
async function fixture(t) {
  ...
  const call = async (name, args) => {
    ...
    if (result.structuredContent.observation) assert.equal(result.structuredContent.update.kind, 'snapshot');
...
test('HTTP modern metadata, discovery, unchanged tool catalog and wire validation', async t => {
  ...
  const encoded = await rpc('tools/call', { name: 'protocol_describe' }, {
    headers: { 'Mcp-Name': `=?base64?${Buffer.from('protocol_describe').toString('base64')}?=` },
  });
...
test('HTTP concurrent games have dedicated state directories, exact receipts and explicit resume', { timeout: 60000 }, async t => {
  ...
  const args = { sessionId: a.sessionId, requestId: 'prayer', expectedRevision: a.revision };
  ...
  assert.deepEqual(await f.call('game_pray', args), prayer);
  const bad = await f.call('game_wait', { ...args, requestId: 'invalid', unexpected: true });
  assert.equal(bad.error.code, 'invalidParams', 'unknown arguments still reach strict C validation');
  ...
  const decline = await f.call('decision_answer', { sessionId: ra.sessionId, requestId: 'decline',
    expectedRevision: ra.revision, decisionId: ra.decision.id, answer: { kind: 'confirmation', confirm: false } });
