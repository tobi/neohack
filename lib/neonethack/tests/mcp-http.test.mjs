...
test('HTTP concurrent games have dedicated state directories, exact receipts and explicit resume', { timeout: 60000 }, async t => {
  ...
  const decline = await f.call('decision_answer', { sessionId: ra.sessionId, decisionId: ra.decision.id, answer: { kind: 'confirmation', confirm: false } });
...
for(const version of ['2025-03-26','2025-06-18','2025-11-25']) test('HTTP earlier handshake and gameplay '+version,async t=>{
  ...
  const created=await send('tools/call',{name:'session_create',arguments:{name:'Earlier',role:'wizard',seed:42}});
  ...
  const observed=await send('tools/call',{name:'session_observe',arguments:{sessionId:game.sessionId}});
...
test('HTTP destination walking crosses the old eight-action limit',async t=>{
  const {call}=await fixture(t);
  const game=await call('session_create',{name:'Range',role:'valkyrie',seed:1});
