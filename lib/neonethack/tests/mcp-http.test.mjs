...
async function fixture(t) {
  ...
  const call = async (name, args) => {
    ...
    if(!presentation)assert.ok(validate(low),JSON.stringify(validate.errors));else {assert.equal(presentation.kind,'compact');assert.equal(low.observation.world,undefined);assert.equal(low.observation.neighborhood,undefined);assert.equal(typeof low.map.text,'string');delete low.map;}
...
test('HTTP concurrent games have dedicated state directories, exact receipts and explicit resume', { timeout: 60000 }, async t => {
  ...
  const bBefore = await f.call("syncState",{sessionId:b.sessionId});
  const args = { sessionId: a.sessionId };
  const [prayer, observed] = await Promise.all([f.call("pray", args), f.call("syncState", { sessionId: b.sessionId })]);
  ...
  assert.deepEqual((await f.call("syncState",{sessionId:b.sessionId})).observation,bBefore.observation);
  assert.deepEqual(perceivedFrame((await f.call('receipt', {...args,operationId:prayer.operationId})).receipt.observation),perceivedFrame(prayer.observation));
  ...
  assert.ok((await f.call("syncState", { sessionId: a.sessionId })).error);
...
for(const version of ['2025-03-26','2025-06-18','2025-11-25']) test('HTTP earlier handshake and gameplay '+version,async t=>{
  ...
  const observed=await send('tools/call',{name:"syncState",arguments:{sessionId:game.sessionId}});
