...
test('concurrent native creates publish one immutable engine cache pin and distinct game directories', { timeout: 60000 }, async t => {
  ...
  for (let round = 0; round < 8; round++) {
    ...
    try {
      const results = await Promise.all(Array.from({ length: 4 }, () => server.call("create", identity)));
...
test('HTTP disconnect preserves accepted input and SIGTERM drains a pending decision answer', { timeout: 30000 }, async t => {
  ...
  const a = (await server.call("create", identity)).structuredContent;
  ...
  const lost = server.call("pray", args, abort.signal);
  const rejected = assert.rejects(lost, /abort/i);
  await delay(30);
  abort.abort();
  await rejected;
  process.kill(pid, 'SIGCONT');
  const prayer = (await server.call("observe", { sessionId: a.sessionId })).structuredContent;
  assert.equal(prayer.decision.kind, 'confirmation');
  assert.equal((await server.call("recover", args)).structuredContent.revision, prayer.revision);
  process.kill(pid, 'SIGSTOP');
  const answer = server.call("answer", { sessionId: a.sessionId, decisionId: prayer.decision.id, value: false });
  await delay(30);
...
test('stalled/crashed C worker cannot block another game; overlapping HTTP calls preserve exact receipts', { timeout: 30000 }, async t => {
  ...
  t.after(() => rm(sessionsPath, { recursive: true, force: true }));
  const server = await fixture(t, sessionsPath);
  const [a, b] = (await Promise.all([server.call("create", identity), server.call("create", identity)])).map(r => r.structuredContent);
  const pid = await owner(server.process.pid, a.sessionId);
  process.kill(pid, 'SIGSTOP');
  t.after(() => { try { process.kill(pid, 'SIGCONT'); } catch {} });
  const args = { sessionId: a.sessionId };
  let completed = false;
  const pending = server.call("pray", args).then(r => { completed = true; return r; });
  const receipt = server.call("recover", args);
  const observed = await server.call("observe", { sessionId: b.sessionId });
  ...
  assert.deepEqual(perceivedFrame((await server.call("recover", args)).structuredContent.observation),perceivedFrame(prayer.structuredContent.observation), 'recovery after completion always reads the exact receipt');
  process.kill(pid, 'SIGSTOP');
  const uncertain = server.call("observe", { sessionId: a.sessionId });
  ...
  assert.equal((await server.call("observe", { sessionId: b.sessionId })).isError, false);
  const unloaded = await server.call("observe", { sessionId: a.sessionId });
  assert.equal(unloaded.isError, true, 'observations never implicitly resume a crashed worker');
  const resumed = await server.call("resume", { sessionId: a.sessionId });
...
for(const mode of ['http','stdio'])test(`${mode}: close bounds idle stopped worker shutdown and releases its run for a fresh server`, {timeout:15000}, async t=>{
  ...
  const created=(await server.call("create",identity)).structuredContent;
  const second=(await server.call("create",identity)).structuredContent;
  ...
  const other=(await independent.call("create",identity)).structuredContent;
  let stopped=true;
  ...
  try {
    ...
    const unaffected=(await independent.call("wait",{sessionId:other.sessionId})).structuredContent;
    ...
    const resumed=(await fresh.call("resume",{sessionId:created.sessionId})).structuredContent;
...
test('shutdown waits for accepted navigation before starting idle retirement grace', {timeout:15000}, async t=>{
  ...
  try {
    ...
    await closing;
    const fresh=await fixture(t,sessionsPath);
    const resumed=(await fresh.call("resume",sid)).structuredContent;
