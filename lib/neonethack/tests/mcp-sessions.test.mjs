...
test('a blocked SQLite writer cannot block another game worker',async t=>{
  ...
  await once(locker.stdout,'data');let completed=false;
  const waiting=f.call('search',{sessionId:a.sessionId,turns:1}).then(r=>{completed=true;return r;});
  const other=await f.call('search',{sessionId:b.sessionId,turns:1});
  assert.equal(other.error,undefined);assert.equal(completed,false,'other game completes while first journal cannot write');
...
test('lost HTTP response retains one accepted input and recovery is read-only',async t=>{
  ...
  // sees no operation ID, then asks the shared adapter for its retained receipt.
  let hidden;
  ...
  const recovered=await f.call('recover',sid);assert.equal(recovered.operationId,hidden.operationId);assert.equal(recovered.historical,true);
  assert.equal(recovered.outcome.turnsElapsed,1);assert.equal(recovered.observation.turn,a.observation.turn+1);
  assert.equal((await f.call('observe',sid)).observation.turn,recovered.observation.turn);
  await f.restart();await f.call('resume',sid);
  const exact=await f.call('receipt',{...sid,operationId:recovered.operationId});assert.deepEqual(exact,recovered);
});
