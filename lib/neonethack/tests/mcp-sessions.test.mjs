...
test('lost HTTP response retains one accepted input and recovery is read-only',async t=>{
  ...
  const wire=await send;const hidden=(await wire.json()).result.structuredContent;
  const recovered=await f.call('syncState',sid);assert.equal(recovered.verification,undefined);
  assert.equal(hidden.outcome.turnsElapsed,1);assert.equal(recovered.observation.turn,a.observation.turn+1);
  assert.equal((await f.call('syncState',sid)).observation.turn,recovered.observation.turn);
...
test('unknown saved token has a structured error without creating a run',async t=>{
  const f=await mcpClient(t,'stdio');
  const result=await f.call('resume',{sessionId:'MissingRun123456'});
  assert.equal(result.error.code,'sessionNotFound');
