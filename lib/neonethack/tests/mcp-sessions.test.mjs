...
test('HTTP disconnect preserves accepted input and SIGTERM drains a pending decision answer', { timeout: 30000 }, async t => {
  ...
  const args = { sessionId: a.sessionId, requestId: 'disconnected-prayer', expectedRevision: a.revision };
  ...
  assert.equal((await server.call('game_pray', args)).structuredContent.revision, prayer.revision);
  process.kill(pid, 'SIGSTOP');
  const answer = server.call('decision_answer', { sessionId: a.sessionId, requestId: 'drained-answer',
    expectedRevision: prayer.revision, decisionId: prayer.decision.id, answer: { kind: 'confirmation', confirm: false } });
  await delay(30);
...
test('stalled/crashed C worker cannot block another game; same-game calls queue and preserve exact receipts', { timeout: 30000 }, async t => {
  ...
  const receipt = server.call('game_pray', args);
  ...
  assert.deepEqual(await receipt, prayer, 'queued exact retry returns the same receipt');
  process.kill(pid, 'SIGSTOP');
  ...
  assert.deepEqual(await server.call('game_pray', args), prayer);
});
