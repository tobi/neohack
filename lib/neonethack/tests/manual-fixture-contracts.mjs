...
export async function manualFixtureContracts(t,fixture){
  await t.test('only visible, non-hallucinatory deaths emit located appearance events',async t=>{
    for(const name of ['DeathArt','DeathArtHidden','DeathArtHallucinated']) {
      const {api}=await fixture(t);const g=await api.create({...identity,name});
      const events=[];
      for(let i=0;i<8;i++){const result=await g.attack('east');events.push(...result.events.filter(e=>e.type==='creatureDied'));}
      if(name==='DeathArt') {
        assert.equal(events.length,1);assert.equal(events[0].x,41);assert.equal(events[0].y,10);
        assert.equal(events[0].appearance,'newt');assert.equal(events[0].levelId,g.observation.location.id);
      } else assert.equal(events.length,0,name);
      assert.equal((await g.observe()).events.filter(e=>e.type==='creatureDied').length,0);
    }
  });
  await t.test('a visible lifesaving rescue does not announce a death',async t=>{
    const {api}=await fixture(t);const g=await api.create({...identity,name:'DeathArtSaved'});
    let rescued=false;
    for(let i=0;i<8&&!rescued;i++){
      const result=await g.attack('east');
      assert.equal(result.events.filter(e=>e.type==='creatureDied').length,0);
      rescued=result.events.some(e=>e.type==='heard'&&/medallion crumbles/.test(e.text));
    }
    assert.ok(rescued);assert.ok(g.observation.world.find(c=>c.x===41&&c.y===10)?.occupant);
  });
  await t.test('statue subjects and weapon shapes agree across perceived world, local and inventory views',async t=>{
    ...
    for(const [x,appearance] of [[39,'chest'],[40,'large box'],[41,'ice box']]) {
      assert.deepEqual(cell(x,12).objects[0].known,{appearance});
      assert.equal(cell(x,12).objects[0].category,'tool');
    }
