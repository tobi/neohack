...
export async function manualFixtureContracts(t,fixture){
  await t.test('known properties and menu-equivalent spell/skill information are pure; hidden charges and curses remain absent',async t=>{
    ...
    assert.deepEqual(unknown(a).known,{});assert.deepEqual(unknown(a),unknown(b));
  ...
  await t.test('quiver, fire, swapping and two-weapon controls preserve genuine item/target choices',async t=>{
    const {api}=await fixture(t);const g=await api.create(identity);const daggers=named(g,'selected-daggers');await g.quiver(daggers);assert.ok(g.observation.inventory.find(i=>i.id===daggers.id).usage.includes('quivered'));
    ...
    await g.quiver();assert.equal(g.decision?.kind,'item');await g.answer(g.decision.id,{kind:'item',item:{id:'hands'}});assert.ok(!g.observation.inventory.some(i=>i.usage.includes('quivered')));
  ...
  await t.test('artifact invocation changes real levitation and two-weapon mode uses explicitly prepared slots',async t=>{
    ...
    assert.ok(g.observation.inventory.some(i=>i.usage.includes('offhand')));
    await g.twoWeapon();assert.ok(!g.observation.inventory.some(i=>i.usage.includes('offhand')));
  });
}
