...
for(const fault of ['worker-death','disconnect'])for(const action of ["wait",'explore',"answer"])test(`supervisor recovers ${action} after committed input and ${fault}`, {timeout:30000}, async t=>{
  ...
  const question=action==="answer"?await call("pray",sid):null;
  ...
  const pending=call(action,action==='explore'?{...sid,maxActions:3}:question?{...sid,decisionId:question.decision.id,value: false}:sid,cancellation.signal).catch(error=>({transportError:error.name}));
  let stopped=false;
  for(let i=0;i<500;i++){
  ...
  if(fault==='worker-death'){
    ...
    assert.equal((await call("wait",sid)).error.code,'uncertainExecution','no new input while recovery is unresolved');
  ...
  const [retry,read]=await Promise.all([call("recover",sid),call("observe",sid)]);
  ...
  const next=await call("wait",sid);assert.ok(!next.error);assert.equal(next.observation.turn,retry.observation.turn+1);
});
