...
for(const boundary of ['reserve','completion'])test('WASM disk journal survives actual SQLite '+boundary+' failure and process death',async t=>{
  ...
  if(boundary==='completion'){
    const receipt=await f.call('receipt',{...sid,operationId:failed.operationId});assert.equal(receipt.outcome.turnsElapsed,1);assert.equal(receipt.observation.turn,restored.observation.turn);
    ...
    assert.deepEqual(receipt.observation,expected.observation);assert.deepEqual(receipt.outcome,expected.outcome);assert.deepEqual(receipt.events,expected.events);assert.equal(receipt.operationId,expected.requestId);
    const {neighborhood,...observed}=(await f.call('observe',sid)).observation;assert.deepEqual(observed,restored.observation);
...
test('crash restart retains exact standing question, receipt and original runtime despite changed default',async t=>{
  ...
  const {neighborhood,...receiptObservation}=(await f.call('receipt',{...sid,operationId:prayer.operationId})).observation;assert.deepEqual(receiptObservation,prayer.observation);
...
test('two servers cannot own one disk journal; unrelated games remain usable',async t=>{
  ...
  assert.equal((await f.call('observe',{sessionId:a.sessionId})).revision,a.revision);
});
