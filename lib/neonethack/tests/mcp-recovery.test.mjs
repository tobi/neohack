...
for(const fault of ['worker-death','disconnect'])for(const action of ['game_wait','explore'])test(`supervisor recovers ${action} after committed input and ${fault}`, {timeout:30000}, async t=>{
  ...
  const pending=call(action,action==='explore'?{...sid,maxActions:3}:sid,cancellation.signal).catch(error=>({transportError:error.name}));
  let stopped=false;
  for(let i=0;i<500;i++){
  ...
  assert.ok(elapsed>=1 && elapsed<=(action==='explore'?3:1));
  ...
  assert.deepEqual(retry.observation,resumed.observation);
