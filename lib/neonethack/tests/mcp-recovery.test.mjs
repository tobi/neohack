...
for(const boundary of ['reserve','completion'])test('WASM disk journal survives actual SQLite '+boundary+' failure and process death',async t=>{
  ...
  if(boundary==='completion'){
    ...
    const observed=(await f.call('observe',sid)).observation;assert.deepEqual(observed,restored.observation);
...
test('crash restart retains exact standing question, receipt and original runtime despite changed default',async t=>{
  ...
  const {neighborhood,...receiptObservation}=(await f.call('receipt',{...sid,operationId:prayer.operationId})).receipt.observation;assert.deepEqual(receiptObservation,prayer.observation);
