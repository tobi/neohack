...
async function fixture(t) {
  ...
  async function connect(args = []) {
    ...
    clients.push(client);
    readers.set(client,new CompactObservationReader());
  ...
  const call = async (client,name,args) => {
    ...
    assert.equal(result.isError,false,JSON.stringify(result)); return readers.get(client).apply(result.structuredContent);
  };
...
test('relocated static bundle: concurrent cold extraction, distinct games, exact receipts and resume', {timeout:60000}, async t => {
  ...
  const [one,two] = await Promise.all([f.call(a,'session_create',identity),f.call(b,'session_create',identity)]);
  ...
  const args = {sessionId:one.sessionId,requestId:'pray',expectedRevision:one.revision};
  const prayer = await f.call(a,'game_pray',args);
  assert.equal(prayer.decision.kind,'confirmation');
  assert.deepEqual(await f.call(a,'game_pray',args),prayer);
  await f.call(a,'session_close',{sessionId:one.sessionId});
  await a.close();
  const restarted = await f.connect();
  const resumed = await f.call(restarted,'session_resume',{sessionId:one.sessionId});
  assert.equal(resumed.decision.id,prayer.decision.id);
  assert.deepEqual(await readFile(`${state}/engine`),pin);
  assert.deepEqual(await f.call(restarted,'game_pray',args),prayer);
  await f.call(restarted,'session_close',{sessionId:one.sessionId});
  await f.call(b,'session_close',{sessionId:two.sessionId});
});
...
test('bundled HTTP starts with only a session path and exposes unchanged tools', {timeout:30000}, async t => {
  ...
  const games = await Promise.all([1,2].map(()=>rpc('tools/call',{name:'session_create',arguments:identity})));
  assert.notEqual(games[0].structuredContent.sessionId,games[1].structuredContent.sessionId);
  for (const game of games) await rpc('tools/call',{name:'session_close',arguments:{sessionId:game.structuredContent.sessionId}});
  await server.close();
});
...
test('single file runs real games in a scratch container without system libraries or account files', {timeout:60000}, async t => {
  ...
  const games = await Promise.all([1,2].map(()=>client.callTool({name:'session_create',arguments:identity})));
  for (const game of games) {
    assert.equal(game.isError,false,JSON.stringify(game));
    assert.ok(game.structuredContent.observation.world.length);
    const closed = await client.callTool({name:'session_close',arguments:{sessionId:game.structuredContent.sessionId}});
