...
test('real headless perception is invariant under hidden locks, traps, disguises, monsters, curses and properties', async t=> {
  ...
  let baseline, appearances, baselineWorld;
  for(const variant of ['baseline','lock','trap','secret','monster','mimic','curse','property','background','legacy','hallucination','peaceful','hostile']) {
    ...
    if(variant==='baseline') baselineWorld=game.observation.world;
    if(variant==='background') assert.deepEqual(game.observation.world,baselineWorld,
      'unseen terrain mutations must not change displayed backgrounds');
