...
for (const [shared, type, generator] of [
  ...
  const mcp = run(`${prefix}/bin/neonethack-mcp`, [`${root}/engine/playground/nethack`, `${root}/engine/playground`, `${dir}/mcp-worlds`], { input: JSON.stringify({jsonrpc:'2.0',id:1,method:'initialize',params:{protocolVersion:'2025-11-25',capabilities:{},clientInfo:{name:'install-test',version:'1'}}})+'\n' });
  assert.equal(JSON.parse(mcp).result.serverInfo.name, 'neonethack');
  ...
  const canary = 'int mj_valid(const char *s) { (void)s; return 0; }\n'
    ...
    + 'const char *const nnh_compass_names[] = {"host compass"};\n'
    + 'const char *const nnh_terrain_names[] = {"host terrain"};\n'
    + ['resolve_cell', 'emit_basis', 'emit_gate', 'emit_cell_actions', 'emit_neighborhood']
      .map(name => `void nnh_${name}(void) {}\n`).join('')
