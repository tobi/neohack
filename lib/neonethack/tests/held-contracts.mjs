...
import {characterStatus} from '../../../web/neohack.dev/src/character-status.ts';
...
export async function heldContracts(t, fixture) {
  ...
  for (const name of ['VisibleHold', 'InvisibleHold']) await t.test(name, async t => {
    ...
    const step = async (method, ...args) => {
      ...
      assert.equal(characterStatus(actual.observation).conditions.split(' · ').includes('held'), isHeld(actual),
        'the shared human HUD/character-sheet formatter agrees with the semantic frame');
      return actual;
    };
...
export async function heldNavigationContract(call) {
  ...
  assert.deepEqual(receipt.observation.vitals, held.observation.vitals);
