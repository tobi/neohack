...
export async function exerciseAgentSyntax(call) {
  const invoke=async(name,args={})=>{const r=await call(name,args);assert.ok(!r.error,JSON.stringify({name,error:r.error}));return r;};
  ...
  assert.equal((await invoke('observe',sid)).revision,state.revision);
  ...
  assert.deepEqual((await invoke('observe',sid)).decision,read.decision);
  ...
  const afterInvalid=await invoke('observe',bag);assert.equal(afterInvalid.revision,state.revision);
  ...
  assert.equal((await invoke('recover',bag)).operationId,state.operationId);
