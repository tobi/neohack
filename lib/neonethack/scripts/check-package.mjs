...
export const workers = ['assets.mjs', 'core-worker.mjs', 'engine-worker.mjs', 'worker-port.mjs'];
...
export async function checkPackage(base = root) {
  ...
  const allowed = new Set([...compiled, ...workers, ...notices, 'manifest.json', 'build.json']);
  for (const name of await readdir(directory)) if (!allowed.has(name)) throw Error(`Unexpected WASM artifact: ${name}`);
  ...
  for (const file of await walk(`${base}/dist`)) {
    ...
    if (!/^(typescript\/(client|types|requests|native|wasm)|mcp\/(cli|server)|protocol\/(catalog|response))\.(js|js\.map|d\.ts)$/.test(file)) throw Error(`Unexpected dist file: ${file}`);
  ...
  for (const module of ['typescript/client', 'typescript/native', 'typescript/wasm', 'mcp/server', 'mcp/cli']) for (const ext of ['js', 'd.ts']) await readFile(`${base}/dist/${module}.${ext}`);
