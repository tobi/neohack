...
// Receipt binding adds an atomic private metadata reservation before append.
// Measure that filesystem operation separately from hash/whole-action timing.
const reservations=[];
const metadata=await readFile(`${b.sessions}/${game.id}/meta.json`);
const parent=await open(dir,'r');
try {
  for(let i=0;i<100;i++) {
    const start=performance.now(),temp=join(dir,'meta.pending');
    const handle=await open(temp,'wx',0o600);
    try { await handle.writeFile(metadata);await handle.sync(); }
    finally { await handle.close(); }
    await rename(temp,join(dir,'meta.json'));await parent.sync();
    reservations.push(performance.now()-start);
  }
