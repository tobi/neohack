...
export function safePath(path) {
  if (typeof path !== 'string' || !path || path.startsWith('/') || path.includes('\\') || path.split('/').some(s => !s || s === '.' || s === '..') || /[\x00-\x1f\x7f]/.test(path)) throw Error(`Unsafe source path: ${path}`);
  return path;
}
export function sourcePath(path) {
  ...
  if ((!template && /(^|\/)(\.git|node_modules|playground|build|dist|sessions|__pycache__)(\/|$)/.test(path)) || /\.(o|a|so(?:\.\d+)*|bc|wasm|pyc)$/.test(path)) throw Error(`Non-source path: ${path}`);
}
...
export async function sourceFiles(base = root) {
  ...
  for (const dir of ['src', 'include', 'cli', 'protocol', 'typescript', 'mcp', 'wasm', 'scripts', 'tests', 'docs']) {
    for (const path of await walk(`${base}/${dir}`)) {
      ...
      const bundleRecipe = dir === 'scripts' && ['Containerfile.bundle', 'bundle.ignore'].includes(path);
      if (!vector && !document && !bundleRecipe && !/\.(c|h|inc|ts|mts|mjs|md|json|sh|txt|in|cmake)$/.test(path) && path !== 'CMakeLists.txt') throw Error(`Unexpected source file: ${dir}/${path}`);
      files.push(`${dir}/${path}`);
    }
  }
  for (const path of files) {
    // Apply the archive policy before reading/snapshotting any selected bytes,
    // including inventory entries and otherwise plausible .json/.c files.
    sourcePath(`lib/neonethack/${path}`);
    await regularSource(base, path, directories);
  }
