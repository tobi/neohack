 ...
 * explicit mapping. Runtime contract tests prove those mappings execute. */
export function operationCoverage(operations, mappings) {
  ...
  for (const name of known) if (!covered.has(name)) errors.push(`uncovered operation: ${name}`);
  return errors;
}
...
export async function checkTools({ web, http, scripts = true } = {}) {
  ...
  const coverageErrors = operationCoverage(catalog.methods, agentMethods);
  results.push({surface: 'agent operation mappings', count: catalog.methods.length, ok: !coverageErrors.length, errors: coverageErrors,
    note: 'Declared coverage; engine-backed agent tests separately verify guards, navigation and recovery.'});
