...
import { root, sha256, sourceState, safePath, walk } from './source-files.mjs';
import { readTar, extract, sourcePath, hostPathLeak } from './archive.mjs';
...
for (const path of native.keys()) assert.match(path, nativeAllowed);
...
for (const [path, entry] of npm) {
  if (path.startsWith('dist/')) continue;
  assert.match(path, /^(package\.json|README\.md|NOTICE\.md|include\/neonethack\.h|docs\/[^/]+\.md|protocol\/(?:catalog|request\.schema|response\.schema)\.json)$/);
  assert.deepEqual(JSON.stringify(path === 'package.json' ? json(npm, path) : entry.data.toString()),
    JSON.stringify(path === 'package.json' ? json(sources, `lib/neonethack/${path}`) : sources.get(`lib/neonethack/${path}`)?.data.toString()), `Packed source differs: ${path}`);
}
...
for (const [installed, source] of [
  ...
  ['share/neonethack/license', 'engine/dat/license'],
...
await writeFile(`${consumer}/package.json`, JSON.stringify({ private: true, type: 'module', dependencies: { neonethack: `file:${out}/verified-npm.tgz`, '@modelcontextprotocol/sdk': metadata.dependencies['@modelcontextprotocol/sdk'] } }, null, 2));
...
await writeFile(`${out}/AUDIT.json`, JSON.stringify({ publicationApproved: false, sourceHash: preview.sourceHash, archivedBuildId: preview.buildId, rebuiltBuildId: rebuilt.buildId, sourceFilesVerified: Object.keys(manifest.files).length, archives: preview.archives, checks: ['archive-structure-hashes-inventories', 'installed-native-wasm-mcp-consumers', 'installed-cmake-c-consumer', 'extracted-source-native-wasm-browser-rebuild'] }, null, 2) + '\n');
