...
export class ProtocolUploader {
  constructor({
    ...
    Object.assign(this, { store, headers, url, token, active, status, register });
  ...
  async pump() {
    for (const id of this.dirty) {
      ...
      while (ack.count < header.count) {
        ...
        // The exact batch is durable before registration. A registration outage
        // retries this dependency, never a game input or a different batch.
        if (!this.registered.has(id)) {
          await this.register(id, this.lifetime.signal);
          if (this.closed) return;
      ...
      if (
        ...
        if (checkpoint && checkpoint.index <= ack.count) {
          ...
          ack = {
            ...
            manifest: receipt.manifest ?? ack.manifest,
          };
          await this.store.acknowledge(id, ack);
        }
      }
