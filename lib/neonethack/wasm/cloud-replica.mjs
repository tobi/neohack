...
export async function fetchCloudReplica(url) {
  ...
  const response=await fetch(manifestUrl,{signal:AbortSignal.any([deadline,AbortSignal.timeout(3000)])});
  ...
  await Promise.all(Array.from({length:Math.min(4,blocks.length)},async()=>{
    for(;;){const i=next++;if(i>=blocks.length)return;
      ...
      const result=await fetch(part,{signal:AbortSignal.any([deadline,AbortSignal.timeout(10000)])});
