...
let journal, readOnlyMethods=new Set();
const journalHeaders=new Map(), journalLoaded=new Set(), journalPresent=new Set();
let playbackIndex=0;
let playbackScene;
let replayArchive;
...
async function initialize(options) {
  ...
  if (!['memory','indexeddb','journal','filesystem'].includes(storage.kind)) throw Error('Unknown WASM storage kind');
  if (storage.kind !== 'memory' && storage.kind !== 'filesystem') {
  ...
  if(storage.kind==='filesystem'){
    if(!inNode||typeof storage.path!=='string')throw Error('Filesystem journals require a local runtime');
    const {openFileStore}=await import('./file-store.mjs');
    journal=await openFileStore(storage.path);
  }
  ...
  globalThis.__nnhHost = {
    ...
    protocolJournal:storage.kind==='journal'||storage.kind==='filesystem',
...
if (seed) {
  ...
  if(storage.kind==='journal'||storage.kind==='filesystem'){
...
async function journalRequest(request) {
  ...
  if(request.method==='session.create'){
    id=storage.kind==='filesystem'?storage.sessionId:randomRunId();
    if(!/^[A-Za-z0-9_-]{16}$/.test(id))throw Error('Invalid filesystem run identity');
    creation={id,epoch:Math.floor(Date.now()/1000)};
