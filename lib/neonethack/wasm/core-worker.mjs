...
async function initialize(options) {
  ...
  if (storage.kind === 'indexeddb') {
    ...
    if (typeof storage.replicaUrl === 'string' && storage.replicaUrl) {
      try {
        const remote = await fetch(storage.replicaUrl, { signal: AbortSignal.timeout(3000) });
        if (remote.ok) {
    ...
    if (storage.replicaUrl && remoteAvailable) {
      ...
      if (pending && pending.commit !== replicaRevision && pending.base !== replicaRevision) throw Error("Pending cloud upload conflicts with remote progress; local progress retained.");
