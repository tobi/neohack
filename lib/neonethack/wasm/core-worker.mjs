...
let module, manifest, storage, releaseLock;
let booted = false;
let nextHandle = 1;
...
const syncfs = populate => new Promise((resolve, reject) => module.FS.syncfs(populate, error => error ? reject(error) : resolve()));
...
async function initialize(options) {
  ...
  globalThis.__nnhHost = {
    ...
    async sync() { if (storage.kind === 'indexeddb') await syncfs(false); },
    start: startEngine,
    write(handle, line) {
      const entry = engines.get(handle);
  ...
  if (storage.kind === 'indexeddb') {
    fs.mount(module.IDBFS, {}, sessions);
    await syncfs(true);
    // IDBFS reconciles files using millisecond mtimes. Make every write's mtime
    // strictly increasing so two durable boundaries in one millisecond cannot
    // collapse into an unchanged remote file. Do not mutate untouched files.
    const write = fs.write;
    let clock = Date.now();
    fs.write = function(stream, ...args) {
      const n = write.call(this, stream, ...args);
      if (n > 0) stream.node.mtime = stream.node.ctime = clock = Math.max(Date.now(), clock + 1);
      return n;
    };
  }
  fs.mkdir('/template');
...
async function close() {
  try {
    if (module) await module.ccall('nnh_wasm_close', null, [], [], { async: true });
  } finally {
...
listen(message => {
  tail = tail.then(async () => {
    ...
    if (message.type === 'init') result = await initialize(message.options ?? {});
      ...
      const response = await module.ccall('nnh_wasm_request', 'string', ['string'], [json], { async: true });
