...
const REPLICA_IDLE_MS = 5000;
let replicaDue = 0, replicaLastStatus = '';
const replicaStatus = (state, message = '') => {
  const key = state + ':' + message;
  if (key === replicaLastStatus) return;
  replicaLastStatus = key;
  send({ type: 'replica', state, message });
};
function scheduleReplica() {
  clearTimeout(replicaTimer);
  replicaTimer = setTimeout(() => { void uploadReplica(); }, Math.max(0, replicaDue - performance.now()));
}
function queueReplica() {
  if (!storage.replicaUrl || replicaFailed) return;
  replicaPending = true;
  replicaDue = performance.now() + REPLICA_IDLE_MS;
  if (!replicaFlight) replicaStatus('queued');
  scheduleReplica();
}
async function uploadReplica(flush = false) {
  if (replicaFlight || !replicaPending || replicaFailed) return replicaFlight;
  if (!flush && performance.now() < replicaDue) { scheduleReplica(); return; }
  ...
  replicaFlight = (async () => {
    ...
    replicaStatus(replicaPending ? 'queued' : 'saved');
  })().catch(error => {
    replicaFailed = true;
    ...
    replicaFlight = null;
    if (replicaPending && !replicaFailed) scheduleReplica();
  });
