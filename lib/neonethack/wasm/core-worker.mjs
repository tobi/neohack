...
async function initialize(options) {
  ...
  globalThis.__nnhHost = {
    ...
    async sync() {
      ...
      await store.sync();
      // Replica is best-effort. A cloud failure must not poison local fsync
      // (human and WebMCP share this path).
      if (typeof storage.replicaUrl !== 'string' || !storage.replicaUrl) return;
      try {
        const put = await fetch(storage.replicaUrl, {
          method: 'PUT',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify(await store.snapshot()),
        });
        if (!put.ok) diagnostic(`Cloud replica HTTP ${put.status}`);
  ...
  if (storage.kind === 'indexeddb') {
    ...
    if (typeof storage.replicaUrl === 'string' && storage.replicaUrl) {
      try {
        ...
        if (remote.ok) seed = await remote.json();
      } catch (error) { diagnostic(error); }
    }
