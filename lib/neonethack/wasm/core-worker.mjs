...
if (seed) {
  ...
  replicaFiles = seed.files;
    ...
    if (storage.replicaUrl && remoteAvailable) {
      ...
      replicaConflict = !!(pending && pending.commit !== replicaRevision && pending.base !== replicaRevision) || cursor !== replicaRevision;
      if (replicaConflict) send({type:"replica",state:"error",message:"Local progress conflicts with remote progress; cloud sync paused. Local play remains available."});
    }
  }
  if (storage.replicaUrl && store && !replicaConflict) {
    replica = new ReplicaUploader({store,url:storage.replicaUrl,revision:replicaRevision,files:replicaFiles,
      active:()=>requestActive,status:(state,message='')=>send({type:'replica',state,message})});
    replica.queue();
  }
  fs.mkdir('/template');
