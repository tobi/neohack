...
import {inputManifest,inputRecords,validateManifest} from './protocol-reader.mjs';
...
async function restoreJournal(id) {
  ...
  if(!header&&storage.archiveConfigUrl){
    ...
    const url=new URL('replays/'+id+'/manifest.json',base.href.endsWith('/')?base.href:base.href+'/');
...
listen(message => {
  ...
  tail = tail.then(async () => {
    ...
    if (message.type === 'init') result = await initialize(message.options ?? {});
    ...
    } else if(message.type==='playbackBatch') {
      ...
      globalThis.__nnhHost.protocolJournal=true;
      for(const record of message.records){validateRecord(record,playbackIndex);result=await replayRecord(record);playbackIndex++;
        if(result.observation)playbackScene=result;
