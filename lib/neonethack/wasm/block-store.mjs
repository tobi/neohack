...
/** Header-only existence probe: no journals or blocks are read on the UI thread. */
export async function hasStoredSession(name, sessionId) {
  if (!sessionId) return false;
  const opening=indexedDB.open(databaseName(name));
  opening.onupgradeneeded=()=>opening.transaction.abort();
  let db;
  try {db=await request(opening);}catch(error){if(error?.name==='AbortError')return false;throw error;}
  try {
    if(db.version!==23||!db.objectStoreNames.contains('files'))return false;
    const tx=db.transaction('files','readonly');
    return (await request(tx.objectStore('files').getKey(databaseName(name)+'/'+sessionId+'/meta.json')))!==undefined;
  }finally{db.close();}
}
