...
export async function openBlockStore(module, mountpoint, name, seed) {
  ...
  try {
    const opening = indexedDB.open(databaseName(name), 22);
    ...
    if (seed) {
      if (seed.version !== 1 || !Array.isArray(seed.files) || !Array.isArray(seed.blocks))
        throw Error('Invalid cloud replica');
      const empty = await request(db.transaction('files').objectStore('files').count());
      if (!empty) {
        const plant = db.transaction(['files', 'blocks'], 'readwrite', { durability: 'strict' });
        const planted = committed(plant);
        try {
          const metadata = plant.objectStore('files'), data = plant.objectStore('blocks');
          for (const [id, block] of seed.blocks) {
            if (typeof id !== 'string' || !block || typeof block.data !== 'string') throw Error('Invalid cloud replica block');
            data.put({ version: block.version, size: block.size, codec: block.codec, data: b64decode(block.data) }, id);
          }
          for (const [path, entry] of seed.files) metadata.put(entry, path);
        } catch (error) { plant.abort(); await planted.catch(() => {}); throw error; }
        await planted;
      }
    }
    const tx = db.transaction(['files', 'blocks'], 'readonly'), done = committed(tx);
    const [paths, headers, ids, blocks] = await Promise.all([
    ...
    for (let i = 0; i < ids.length; i++) decoded.set(ids[i], await decodeBlock(ids[i], blocks[i]));
  ...
  async function snapshot() {
    ...
    return {
      version: 1,
      files: paths.map((path, i) => [path, headers[i]]),
      blocks: ids.map((id, i) => {
  ...
  return { sync, snapshot, close() { db.close(); } };
}
