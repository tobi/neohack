...
add("session.route", "Find a shortest known walking route in the perceived current level. Free revision-bound query; does not move. knownWalking excludes closed doors, non-ally occupants, boulders, known hazards and uncertain diagonal squeezes; a displayed tame ally is displaceable and may lie on the route. Null distance means no route known under this policy, not impossible or unsafe. why classifies a null distance as targetOccupied, targetUnknown, closedDoor or disconnected. Steps exclude the origin. Recheck after input.", object({ sessionId: sid, expectedRevision: integer(), to: object({ x: integer(1,79), y: integer(0,20) }) }), "route", { readOnly: true, idempotent: true });
