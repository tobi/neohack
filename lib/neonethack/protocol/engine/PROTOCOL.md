...
`../../docs/PROTOCOL.md` for the public semantic interface.

### Runtime profile 1 (2026-09-05)

The public library launches the engine with `--runtime-v1=EPOCH` before option
initialization. Its journaled `initialize.params` adds `runtimeProfile: 1` and
`calendarEpoch: EPOCH` (integer seconds, 0–4102444799). The initialize result must
acknowledge those same fields before the library sends `new_game`. The engine
reports `-32002/E_RUNTIME_PROFILE` for a profile/epoch mismatch. The same arguments
are passed to the WASM engine; this is not a JavaScript calendar implementation.

Profile 1 uses a fixed creation calendar in UTC and bypasses user RC/options.
See [runtime policy](../../docs/REPLAY.md). The raw examples below describe
unprofiled standalone-engine mode, retained for existing private clients; the
new public library refuses to resume unprofiled journals and does not rewrite
historical vectors or synthesize missing original settings.
