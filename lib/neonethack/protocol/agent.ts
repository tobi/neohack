...
export const agentInstructions='Create or resume a run and pass its short sessionId back. Use go for destinations anywhere on the current level, explore to reveal more of it, and descend to approach known stairs and go down; force:true attempts only an adjacent ordinary move, never a force attack. Attack is separate. Answers and cancellations require the exact returned decisionId; never answer a newer question using an older choice. Answers and warnings stay explicit. The adapter manages operation IDs, revisions and self-contained perceived frames; session_observe includes full local action detail. If execution is uncertain, use retry for the pending exact operation or receipt for its operationId; never repeat a new action blindly. Observe when state changes. help gives brief tool names or one detailed schema.';
