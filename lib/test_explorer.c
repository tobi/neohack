/* libneonethack API_DESING.md section 12 acceptance probe.
 *
 * Exercises the explorer contract against a real engine: named actions,
 * typed decisions, outcomes, retries, and resume. Deterministic under the
 * fixed seed; every game is played through public tools only.
 *
 * Build: cc -Wall -Wextra -Ilib -o /tmp/test_explorer \
 *            lib/test_explorer.c lib/session.c lib/minjson.c lib/explorer.c
 * Run from the project root: /tmp/test_explorer
 */
